...
import { getBatchPosters, getL1BaseFeeCollector, getL1SurplusFeeCollector, getL2BaseFeeCollector } from '../utils/misc'
...
export function getFeeCollectorsCommand(program: Command) {
  program
    ...
    .action(async options => {
      ...
      const l1BaseCollectors = await Promise.all(posters.map(poster => getL1BaseFeeCollector(provider, poster)))
      ...
      str += posters.map((poster, i) => `${poster} -> ${l1BaseCollectors[i]}`).join(`\n${tab}`)
      log.info(str)
    })
}
