...
function getEtherscanUrl(chainId: number) {
  ...
  return assertDefined(urls[chainId], `Etherscan URL for chain ${chainId} not found`);
}
...
export async function getContractCreation(chainId: number, addrs: string[]) {
  return hitApiEndpoint(chainId, `module=contract&action=getcontractcreation&contractaddresses=${addrs.join(',')}`);
}

export async function getAbi(chainId: number, addr: string) {
  return (await hitApiEndpoint(chainId, `module=contract&action=getsourcecode&address=${addr}`))[0].ABI;
}